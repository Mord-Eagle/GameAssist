...
> **Module version:** `1.7.0` on the `AlmanacAssist-v2.0.0-Build` line (`1.6.1` is the retained rollback baseline)<br>
> **Default:** Disabled<br>
...
The `AlmanacAssist-v2.0.0-Build` branch is the duplicate build line for Issue #95. Its 1.7.0 checkpoint retains the Gate 0 repairs for Issues #92 and #93 and adds a read-only SceneResolver snapshot consumed by the Current World dashboard, Scene view, announcements, and public weather announcement. The snapshot labels provenance, unavailable domains, partial terrain, hydrology distinctions, and moon phase versus visibility without writing provider state. Expanded places, travel, phenomena, presets, WorldPacks, temporal contexts, and live Roll20 acceptance remain incomplete. See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
| AlmanacAssist 1.7.0 | **v2.0.0 sandbox candidate; AlmanacAssist-v2.0.0-Build** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, Temperate Lowlands starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The 1.7.0 checkpoint adds a deeply immutable, no-write SceneResolver snapshot with provenance, bounded warnings, explicit unavailable domains, partial terrain/hydrology distinctions, and separate moon phase/visibility for the Current World dashboard, Scene view, and coherent announcements. Issues #92/#93 remain accepted; expanded world domains and live Roll20 acceptance remain pending. |
