...
| Help & Navigation | `!ga-help` lists module help screens; a disabled module still explains its purpose before offering Enable. `!ga-nav` opens the module navigator; `!ga-nav <module>` opens that module's destinations, with an extra organized section step for larger modules. |
...
* **HealthService** – `GameAssist.HealthService` normalizes supported official 2014-PC HP attributes and linked-NPC bar 1 changes, deduplicates linked sheet/token evidence, and verifies GameAssist-owned writes carrying a producer and operation ID. Unexplained Roll20 changes remain unknown instead of being mislabeled as combat damage or healing; ConcentrationAssist may present them as clearly labeled observed HP loss rather than claiming a cause.
* **Suite Navigation** – `!GA-GM` and `!GA-DM` open one private control center for all fifteen feature modules, `!ga-help [module]` opens their help destinations or a short purpose-and-enable screen when one is off, and `!ga-nav` provides direct or layered module navigation without moving module-specific behavior into the core interface. GameAssist command words ignore capitalization and accept either spaces or hyphens, so `!GA STATUS`, `!ga-status`, `!gA gM`, and `!GA-GM` are equivalent forms. Disabled modules offer an Enable action instead of a dead button, and shared control panels use Roll20's familiar default-template presentation.
...
| 🎲 **7 · Try the Table Tools** | Test `!token-assist help`, `!condition help`, `!critfumble menu`, `!concentration --status`, `!HP-Selected`, `!Init-Help`, `!Combat-Help`, `!Welcome`, `!Effect-Guide`, `!Heal-Guide`, and `!Attack-Guide` for the modules you use. |
...
```text
Supported 2014 PC HP attribute or linked NPC bar 1 changes
          ↓
...
> **Module version:** `0.2.0`<br>
> **Dependency:** HPAssist does **not** require TokenMod.

HPAssist reads `npc=1` and `npc_hpformula` from linked characters, parses `NdM+K` or `NdM-K`, and writes the result to token `bar1_value` and `bar1_max`. When HealthService is enabled, the roll is recorded as one verified initialization or synchronization operation with HPAssist provenance. If the GM deliberately disables HealthService, HPAssist retains its established direct roll behavior; only shared provenance-aware integration is unavailable.
...
Every recipient used for sheet automation must be a token linked through **Represents Character**. A refusal names the affected token and explains that Roll20 setting. Concentration effects also verify the configured ConcentrationAssist marker before writing any target marker or sheet row; a failed dependency leaves no partial Bless, Guidance, Haste, Holy Weapon, or Pass Without a Trace application. A successful result names the source whose concentration was established.
...
The final review offers **Use Sheet Setting**, **Normal**, **Advantage**, and **Disadvantage**. AttackAssist preserves the verified sheet-generated roll formula and official `atk` or `atkdmg` template fields, qualifies every repeating-row reference to the same stable row, and submits the result as the acting character. Documented optional 2014 defaults such as the normal critical range are supplied when Roll20 did not persist them as attributes. Any unknown missing sheet field is reported before a command reaches the sandbox, with a prompt to open and save that attack on the sheet. AttackAssist never temporarily changes the character's saved roll-mode setting. Once one roll button is used, the other buttons from that review expire.

A visible Player Ribbon target is announced only after Roll20 returns the submitted roll. A GM-reviewed hidden or off-page target remains private. Because the later announcement contains no roll, CritAssist sees a supported natural-1 attack exactly once.
...
Config keys: `enabled` and `allowPlayerAttacks`. Source choices, retained requests, and one-use submissions are sandbox-local safety capabilities and are not exported or restored after a restart.
...
|  | `!ga-health` | `[recent\|audit]` | Show HealthService status, the ten newest bounded HP-evidence entries, or a read-only supported-token audit of the Player Ribbon page. |
...
| **Attacks** | `!Attack` / `!Attack-Menu` | selected controlled 2014 PC or guided source choice | Choose one verified repeating attack, a visible map target, and the sheet, Normal, Advantage, or Disadvantage roll mode. |
...
| **Debug** | `!ga-debug damage` | `--amount N [--token ID] [--apply]` | Preview or apply bar1 damage; supported applied HP writes use verified HealthService damage evidence. |
...
`GameAssist.HealthService` is toggleable core infrastructure. Version `1.0.0` supports official D&D 5E by Roll20 2014 PC `hp` attributes and linked NPC token bar 1. Linked PC sheet/token notifications describing the same before-and-after values are published once. GMs can inspect the service through `!ga-health`, view bounded recent evidence with `!ga-health recent`, run the read-only Player Ribbon page check with `!ga-health audit`, or manage optional PC threshold notices with `!ga-health alerts`.
...
| `readToken(tokenOrId)` | Return an immutable supported linked-PC or linked-NPC bar 1 snapshot or `null`. |
...
`GameAssist.ConcentrationAssist` version `0.4.0` retains its marker and lifecycle methods and adds a bounded optional HealthService offer contract:
...
| EffectAssist 2.5.1 | **v2.0.0 sandbox candidate** | Applies supported 2014-sheet rows through sheet workers, separates player caster selection from recipient count, offers Bless totals of 1-3 plus a higher-level 4-11 menu, names unlinked recipients, preflights concentration before target writes, reports the established source dependency, and retains the established player, audit, repair, cast-proposal, and duration workflows. Live Roll20 acceptance remains required. |
...
| AttackAssist 1.0.3 | **v2.0.0 sandbox candidate** | Disabled-by-default official-2014 repeating-attack guidance with stable row identity, compact direct native visible targeting, retained private placement requests, executable sheet/normal/advantage/disadvantage modes, documented optional sheet defaults, preflight refusal for unknown missing fields, one-use rolls, and no damage or combat-state writes is ready for focused Roll20 testing. |
...
The v2.0.0 candidate keeps four disabled-by-default modules on one development line. EffectAssist 2.5.1 supplies source-aware 2014-sheet effects, separate caster and recipient-count steps, Bless support through eleven recipients, guarded recipient/concentration preflight, compact `(GA)` owned rows, direct application with optional review, worker-safe projection cleanup, one-concentration-by-default behavior, bounded Bless proposals, guarded Guidance consumption, duration candidates, retained GM requests, and a direct GM casting surface. HealAssist 1.0.0 adds a guarded roll-review-confirm path for supported official-2014 healing. AttackAssist 1.0.3 adds stable official-2014 repeating-attack selection, complete optional-placeholder translation, and executable direct native targeting without resolving damage. AlmanacAssist 1.6.0 remains in development under Issue #90 for complete custom-calendar and world-context acceptance before v2.0.0 can ship.
