...
| TokenAssist 1.1.0 | **Implemented and accepted** | General token controls with bare `!token`, full-name `!tokenassist`, `!token-assist`, and `!ta` commands, longest-name-first alias routing, temporary support for older `!token-mod` macros, MarkerService-backed markers, token-change observation, clear compatibility limits, duplicate-install protection, a compact action-focused GM/DM screen, an organized extended action library, a stable manual, and the shared Roll20 default-template presentation. |
