...
> **Module version:** `1.10.0` on the `AlmanacAssist-v2.0.0-Build` line (`1.6.1` is the retained rollback baseline)<br>
> **Default:** Disabled<br>
...
The `AlmanacAssist-v2.0.0-Build` branch is the duplicate build line for Issue #95. Its 1.10.0 checkpoint retains the Gate 0 repairs, read-only SceneResolver, and bounded generic Worldbuilding model, then adds owner-authored Prepared Destinations, bidirectional Travel Routes, and explicit reviewed Phenomena overlays. A Prepared Destination reviews a Location-derived context without replacing Weather, Environment overrides, Astronomy, or fictional time. Travel starts and every route/pace segment is reviewed; fictional time advances and the final Location changes only after explicit acceptance. Phenomena add separate scoped visibility/terrain/travel evidence and never replace Weather, Environment, Astronomy, Climate, or Time. Preset registry, WorldPacks, temporal contexts, and live Roll20 acceptance remain incomplete. See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
| AlmanacAssist 1.10.0 | **v2.0.0 sandbox candidate; AlmanacAssist-v2.0.0-Build** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, Temperate Lowlands starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The 1.10.0 checkpoint retains the no-write SceneResolver and bounded generic world model, adds Location-bound Prepared Destinations, bidirectional Travel Routes, and reviewed explicit Phenomena overlays. Phenomena may be global or Location-scoped, can carry optional fictional-time expiry, and remain separate provenance-marked evidence that does not replace Weather/Environment/Astronomy/Climate/Time authority. Preset registry, WorldPacks, temporal contexts, and live Roll20 acceptance remain pending. |
