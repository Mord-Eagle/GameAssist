...
The `AlmanacAssist-v2.0.0-Build` line is the duplicate build line for Issue #95 and the active AlmanacAssist implementation is **v2.0.0**. The in-progress v2.0.0 code retains the Gate 0 repairs, read-only SceneResolver, bounded generic Worldbuilding, owner-authored Prepared Destinations, bidirectional Travel Routes, explicit reviewed Phenomena overlays, and the generic PresetRegistry campaign-clone workflow. A Prepared Destination reviews a Location-derived context without replacing Weather, Environment overrides, Astronomy, or fictional time. Travel starts and every route/pace segment is reviewed; fictional time advances and the final Location changes only after explicit acceptance. Phenomena add separate scoped visibility/terrain/travel evidence and never replace Weather, Environment, Astronomy, Climate, or Time. WorldPacks are now bounded inert handout data with canonical export/template, syntax/schema/reference/provenance review, expiring confirmation, atomic Worldbuilding-plus-registry commits, no-overwrite New/Copy policy, and guarded higher-version updates. Temporal contexts remain to be built. RulesAdvisor is an optional, profile-specific, advisory-only v2.0.0 feature. **Live Roll20 validation is intentionally deferred until the complete Issue #96 code program is built; it is the final release-acceptance phase, not an in-progress implementation activity.** See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
* `!aa-world` or `!world` → Open generic Worldbuilding Mode for Regions, Geography, Ecoregions, Biomes, Locations, Prepared Destinations, Travel Routes, and Phenomena.
...
| AlmanacAssist v2.0.0 | **Active v2.0.0 implementation build; AlmanacAssist-v2.0.0-Build** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The v2.0.0 build includes the no-write SceneResolver, bounded generic world model, Location-bound Prepared Destinations, bidirectional Travel Routes, reviewed explicit Phenomena overlays, generic reviewed PresetRegistry campaign clones, and optional profile-specific advisory-only RulesAdvisor reminders. Phenomena remain separate provenance-marked evidence that does not replace Weather/Environment/Astronomy/Climate/Time authority. WorldPacks and temporal contexts remain implementation work. Live Roll20 validation begins only after the full Issue #96 code build is complete. |
