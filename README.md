...
* `!aa-world` or `!world` → Open Worldbuilding Mode and the World Library: choose/preview/install a generic Starter World, recover an empty campaign, or manage Regions, Geography, Ecoregions, Biomes, Locations, Prepared Destinations, Travel Routes, and Phenomena. Record editors use Basic, Detailed, and Technical layers; ordinary controls use visible names, while Technical reveals provenance and stable identities without raw JSON.
...
* `!aa-worldpacks`, `!worldpacks`, or `!worldpack` → Create a blank generic WorldPack handout, export portable Worldbuilding records, or choose a visible named handout for an inert New/Update/Copy review before one atomic confirmation. It never imports presets, runtime state, providers, time, weather, astronomy, or gameplay effects.
...
* `!aa-travel` or `!travel` → Plan a route from the current Location, review a pace/distance start, then review each time-advancing segment before it changes fictional time or reaches the destination. If no Location is active, the panel must route the GM to setup rather than requiring a memorized command.
