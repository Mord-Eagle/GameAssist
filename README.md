...
> **Module version:** `1.9.0` on the `AlmanacAssist-v2.0.0-Build` line (`1.6.1` is the retained rollback baseline)<br>
> **Default:** Disabled<br>
...
The `AlmanacAssist-v2.0.0-Build` branch is the duplicate build line for Issue #95. Its 1.9.0 checkpoint retains the Gate 0 repairs, read-only SceneResolver, and bounded generic Worldbuilding model, then adds owner-authored Prepared Destinations and bidirectional Travel Routes. A Prepared Destination reviews a Location-derived context without replacing Weather, Environment overrides, Astronomy, or fictional time. Travel starts and every route/pace segment is reviewed; fictional time advances and the final Location changes only after explicit acceptance. Phenomena, preset registry, WorldPacks, temporal contexts, and live Roll20 acceptance remain incomplete. See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
* `!aa-world` or `!world` → Open generic Worldbuilding Mode for Regions, Geography, Ecoregions, Biomes, Locations, Prepared Destinations, and Travel Routes.
...
| AlmanacAssist 1.9.0 | **v2.0.0 sandbox candidate; AlmanacAssist-v2.0.0-Build** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, Temperate Lowlands starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The 1.9.0 checkpoint retains the no-write SceneResolver and bounded generic world model, adds Location-bound Prepared Destinations and bidirectional Travel Routes, and requires review/confirmation before every Travel start, fictional-time advance, and final arrival location switch. It preserves Weather/Environment/Astronomy ownership and bundles no named setting data. Phenomena, preset registry, WorldPacks, temporal contexts, and live Roll20 acceptance remain pending. |
