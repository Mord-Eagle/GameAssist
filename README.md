...
> **Module version:** `1.8.0` on the `AlmanacAssist-v2.0.0-Build` line (`1.6.1` is the retained rollback baseline)<br>
> **Default:** Disabled<br>
...
The `AlmanacAssist-v2.0.0-Build` branch is the duplicate build line for Issue #95. Its 1.8.0 checkpoint retains the Gate 0 repairs and read-only SceneResolver, then adds bounded generic owner-authored Regions, Geography, Ecoregions, Biomes, and Locations in Worldbuilding Mode. Active Location, favorites, and recents compose into scene provenance, terrain, and hydrology without writing provider state or bundling setting lore. Prepared destinations, reviewed travel, phenomena, presets, WorldPacks, temporal contexts, and live Roll20 acceptance remain incomplete. See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
* `!aa-world` or `!world` → Open generic Worldbuilding Mode for Regions, Geography, Ecoregions, Biomes, and Locations.
* `!aa-location` or `!location` → Choose the current Location, manage favorites, and reopen recent places; Location selection does not silently replace Weather or other provider-owned data.
...
| AlmanacAssist 1.8.0 | **v2.0.0 sandbox candidate; AlmanacAssist-v2.0.0-Build** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, Temperate Lowlands starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The 1.8.0 checkpoint retains the deeply immutable, no-write SceneResolver and adds bounded generic owner-authored Region, Geography, Ecoregion, Biome, and Location records, active-location selection, favorites/recents, Worldbuilding Mode, field-owned terrain/hydrology context, and warning-only future-world schema reads. No named setting data is bundled. Prepared destinations, reviewed Travel, phenomena, presets, WorldPacks, temporal contexts, and live Roll20 acceptance remain pending. |
