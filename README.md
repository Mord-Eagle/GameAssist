...
* **EffectAssist** – starts disabled and coordinates a focused catalog of source-aware effects for the official 2014 sheet. Bless, Guidance, Warding Bond, and Haste receive supported marker and sheet automation; Holy Weapon and Pass Without a Trace are explicitly labeled tracked/manual effects. Players may cast built-ins from controlled sources, use Roll20's map target picker for visible recipients they do not control, or send the GM a compact placement request. Successful player-originated casts are announced publicly only after verification. The GM can lock the player path at any time. Overlapping sources remain independently removable, pre-existing state remains campaign-owned, and audit never writes without fresh confirmation.
...
**GM path:** select the target tokens, run `!effect`, choose an effect, and choose its source. A confirmation panel shows what GameAssist will do automatically and what the table must still handle before anything changes.

**Player path:** run `!effect` or a spell shortcut, choose a controlled casting character, then use Roll20's target prompt to click the visible recipient token or tokens on the map. Players do not need to control or preselect recipients. **Ask the GM** sends a compact request when hidden recipients, unusually large groups, or GM placement are needed. Concentration replacement defaults to **Yes** in the prompt; choose **No** when the new cast should be cancelled instead.
...
Players can run `!effect` or a direct spell shortcut and apply a built-in effect from a linked source token or character they control. Every preview and confirmation rechecks source control, recipient identity, visibility, and page. A player cannot use a crafted target command to affect a hidden or off-page token. A successful player-originated cast is then announced publicly as **Source has cast Effect on Recipients**; direct GM applications remain private. The GM retains the custom-effect, status, audit, repair, and configuration screens and can lock or restore player casting through the Control Center at any time.
...
* `!Bless`, `!Guidance` / `!Guide`, `!Haste`, `!Warding-Bond`, `!Holy-Weapon`, or `!PwoaT` → Open the player-friendly source and map-target path for that effect; commands are case-insensitive.
* `!Effect-Targets` / `!Effect-Request` → Generated player buttons for bounded map targeting or a compact GM placement request; players do not need to memorize these commands.
...
|  | `!effect` / `!Effect-Catalog` | GM selection or player map targets; guided source picker | Open the catalog directly. Bless, Guidance, Warding Bond, and Haste provide supported marker/sheet automation; Holy Weapon and Pass Without a Trace are clearly labeled tracked/manual entries. |
|  | `!Bless` / `!Guidance` / `!Guide` / `!Haste` / `!Warding-Bond` / `!Holy-Weapon` / `!PwoaT` | controlled source; visible map targets | Open a player-friendly source and Roll20 target-picker path for one built-in effect. |
|  | `!Effect-Targets` / `!Effect-Request` | generated buttons | Choose bounded visible recipients or ask the GM to place a player cast without exposing GM-only controls. |
...
| EffectAssist 2.0.0 | **v2.0.0 sandbox candidate** | Disabled-by-default six-effect catalog for the official 2014 sheet, native visible-recipient player targeting, compact GM placement requests, verified public cast announcements, GM lockout, multi-projection ownership, concentration-linked cleanup, bounded history, read-only audit, and confirmed repair are ready for focused Roll20 testing. |
...
5. **Post-v2.0 Health Bridge:** build the shared HealthService contract in Issue #83, then use Issue #79 as its first narrow consumer for trustworthy concentration-check offers after supported HP loss.
6. **v2.y — AlmanacAssist:** use Issue #62 as the master specification and implement Time, Climate, Astronomy, Weather, Environment, and Rest as six separately tracked internal submodule phases.
7. **Later Player and Health Tools:** Issues #84-#87 retain guided healing, verified Guidance consumption, PC health alerts, and guided 2014 attacks without crowding the current release or duplicating HealthService.
8. **v2.z — Deferred Backlog:** revisit older TokenAssist parity work, CombatAssist integrations, and other deferred features after the new module foundations are stable.
...
* Added disabled-by-default EffectAssist 2.0.0 with a focused six-effect launch catalog, player casting from controlled sources, native visible-recipient targeting, compact GM placement requests, verified public cast announcements, GM lockout, source and target records, dependencies, stacking, lifecycle, and bounded history.
