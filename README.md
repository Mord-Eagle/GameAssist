...
The `AlmanacAssist-v2.0.0-Build` line is the duplicate build line for Issue #95 and the active AlmanacAssist implementation is **v2.0.0**. The complete Issue #96 code build includes the Gate 0 repairs, read-only SceneResolver, bounded generic Worldbuilding, owner-authored Prepared Destinations, bidirectional Travel Routes, explicit reviewed Phenomena overlays, generic PresetRegistry campaign clones, optional advisory-only RulesAdvisor, bounded atomic WorldPacks, and explicit regional/planar Temporal Contexts. A Prepared Destination reviews a Location-derived context without replacing Weather, Environment overrides, Astronomy, or fictional time. Travel starts and every route/pace segment is reviewed; fictional time advances and the final Location changes only after explicit acceptance. Phenomena add separate scoped visibility/terrain/travel evidence and never replace Weather, Environment, Astronomy, Climate, or Time. WorldPacks are bounded inert handout data with canonical export/template, syntax/schema/reference/provenance review, expiring confirmation, atomic Worldbuilding-plus-registry commits, no-overwrite New/Copy policy, and guarded higher-version updates. A Temporal Context is a bounded transparent local projection of the one canonical fictional-minute chronology; a reviewed transition advances only that canonical clock and never reverse-writes Rest, effects, NPC history, combat, resources, providers, or real-world records. Worldbuilding record editors consistently offer Basic, Detailed, and Technical layers; technical evidence includes stable identity and provenance without raw JSON in ordinary panels. **The consolidated automated/structural checkpoint—including future Worldbuilding runtime preservation—passed on 2026-08-26. Live Roll20 validation remains the final release-acceptance phase and has not been started in a campaign.** See `docs/AlmanacAssist-v2.0.0-Build/` for the issue assessment and implementation plan.
...
* `!aa-world` or `!world` → Open generic Worldbuilding Mode for Regions, Geography, Ecoregions, Biomes, Locations, Prepared Destinations, Travel Routes, and Phenomena. Record editors use Basic, Detailed, and Technical layers; Technical reveals provenance and stable identities without raw JSON.
* `!aa-phenomena` or `!phenomena` → Review explicit owner-authored Phenomena activation/deactivation, scoped overlay state, and elapsed-duration cleanup.
* `!aa-presets`, `!presets`, or `!preset` → Preview immutable generic Session Preset templates, review an install, and customize the independent campaign-owned clone; aliases accept the established close space/hyphen forms.
...
* `!aa-worldpacks`, `!worldpacks`, or `!worldpack` → Create a blank generic WorldPack handout, export portable Worldbuilding records, or review an inert handout import as New, Update, or Copy before one atomic confirmation. It never imports presets, runtime state, providers, time, weather, astronomy, or gameplay effects.
...
* `!aa-location` or `!location` → Choose the current Location, manage favorites and recents, or review a Location-bound Prepared Destination; Location selection does not silently replace Weather or other provider-owned data.
* `!aa-travel` or `!travel` → Plan a route from the current Location, review a pace/distance start, then review each time-advancing segment before it changes fictional time or reaches the destination.
...
| AlmanacAssist v2.0.0 | **Code complete; final Roll20 acceptance pending** | Disabled-by-default Time, Climate, Astronomy, Weather, Environment, and Rest systems include a Wayfarer-native 20-hour display, independent Off/Descriptive/Detailed/Technical announcement fields, player-perceivable moon visibility, starter context, scene overrides, repaired direct calendar queries, Standard/Heroic/Gritty/Custom rest controls, bounded state/history, audits, semantic events, and transactional 2014-sheet rest safeguards. The complete Issue #96 code build includes the no-write SceneResolver, layered generic Worldbuilding, Prepared Destinations, reviewed Travel, reviewed Phenomena, PresetRegistry, advisory-only RulesAdvisor, inert atomic WorldPacks, explicit temporal contexts, and optional stale-safe Wayfarer handout editing. Phenomena and temporal projections remain distinct evidence; neither replaces Weather/Environment/Astronomy/Climate/Time authority or creates a second clock. The consolidated automated checkpoint is clean; final live Roll20 validation remains required. |
