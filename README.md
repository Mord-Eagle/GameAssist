...
| Flagship GM Commands | `!token-service help`, `!token-mod --help`, `!condition`, `!condition announce`, `!c-a`, `!cond-!`, `!condition help`, `!critfumble`, `!critfumble help`, `!critfumble menu`, `!critfail`, `!npc-hp-all`, `!npc-hp-selected`, `!npc-death-report --help`, `!npc-death-buckets`, `!NPC-WR`, `!npc-death-audit`, `!npc-death-arc`, `!ga-conc-status`, `!ga-config ui`. |
...
* **TokenService** – supplies independently branded general token controls, a practical compatibility surface for supported `!token-mod` macros, explicit-ID permissions, legacy configuration migration, token-change observers, and MarkerService-backed status operations. It refuses duplicate command handling when standalone TokenMod is detected.
...
| 🧩 **2 · Choose Features** | Open `!ga-config ui` and keep only the tools that fit the campaign. MarkerService begins enabled because ConditionService, TokenService, NPCManager, ConcentrationTracker, and marker diagnostics use it. |
...
| 🎲 **6 · Try the Table Tools** | Test `!token-service help`, `!condition help`, `!critfumble menu`, `!concentration --status`, and `!npc-hp-selected` for the modules you use. |
...
This `v0.1.5.0` development line replaces standalone TokenMod and StatusInfo for the token and condition workflows supported by GameAssist. It does not keep a hidden legacy path that sends GameAssist work back to those standalone scripts. Remove both standalone scripts before testing overlapping TokenService or ConditionService commands.

If MarkerService is deliberately disabled, ConditionService, TokenService, NPCManager, ConcentrationTracker, and DebugTools are also disabled. CritFumble, ConfigUI, and NPCHPRoller remain available. Standalone **TokenMod by The Aaron** and **StatusInfo by Robin Kuiper** can then provide their own token-marker and condition tools, but they do not restore GameAssist death-history, concentration, TokenService, or ConditionService features.
...
5. Select a disposable token and use one supported `!token-mod --set` or `--on` command.
...
> **Module version:** TokenService `1.0.0` is an independently branded GameAssist module. It acknowledges TokenMod `0.8.88` by The Aaron, Arcane Scriptomancer, as its compatibility and design foundation, but it is not an official TokenMod release.

TokenService provides general token controls without requiring standalone TokenMod. Its own entry point is `!token-service`; supported `!token-mod` commands remain available so common campaign macros can migrate gradually. Select one or more tokens before running a command. Players can affect tokens they can select, while direct `--ids` targeting remains GM-only unless the GM enables **Players can use --ids**.
...
!token-mod --set name|"City Watch" bar1_value|25
!token-mod --set bar1_value|-5 left|+70
!token-mod --move 3g
!token-mod --set layer|gmlayer --order tofront
!token-mod --set statusmarkers|red:3|Warded;;1001;4
!token-mod --report gm|"{name}: {bar1_value:before} to {bar1_value}"
```
...
TokenService `1.0.0` does **not** claim complete TokenMod compatibility. Image-side stack editing (`imgsrc`/`sides`), default-token writes, computed-attribute handling, advanced color arithmetic, duplicate-index marker operations, and conditional marker counts are intentionally unsupported and return a clear warning. Verify a macro against the supported list before removing a specialized workflow that depends on one of those features.

On first startup, TokenService copies a valid legacy `state.TokenMod.playersCanUse_ids` value into its own configuration. It records the migration and leaves `state.TokenMod` untouched for rollback. It does not expose a global `TokenMod` object; integrations should use `GameAssist.TokenService.observeTokenChange(...)` or MarkerService's marker observer.

Do not leave standalone TokenMod enabled while testing TokenService. Both recognize `!token-mod`. If GameAssist detects standalone TokenMod, TokenService deliberately does not process compatibility commands and warns the GM so one command cannot change a token twice. Remove the standalone script and restart the sandbox.
...
GameAssist v0.1.5.0 replaces standalone TokenMod and StatusInfo for the token and condition workflows supported by TokenService and ConditionService. Remove both standalone scripts before enabling the overlapping GameAssist modules. TokenService and standalone TokenMod both recognize `!token-mod`; ConditionService and standalone StatusInfo both recognize `!condition` and marker changes.

If standalone TokenMod is accidentally left installed, TokenService suspends its compatibility handler and warns the GM instead of applying a command twice. That safeguard is diagnostic, not a supported permanent dual-install arrangement.
...
| **Token Controls** | `!token-service help` / `!token-service about` | — | Open TokenService guidance, compatibility limits, provenance, and attribution. |
|  | `!token-mod --help` / `!token-mod --help-statusmarkers` | — | Open the supported compatibility or marker-command guide. |
|  | `!token-mod --on|--off|--flip <property...>` | selected/authorized targets | Change supported boolean token properties. |
|  | `!token-mod --set <property|value...>` | selected/authorized targets | Change supported token, bar, aura, vision, lighting, layer, position, and marker properties. |
|  | `!token-mod --move <distance|angle\|distance>` | selected/authorized targets | Move tokens using pixels, grid units, or page units. |
|  | `!token-mod --order tofront|toback` | selected/authorized targets | Change token stacking order. |
|  | `!token-mod --report <recipient\|message>` | `{property}` placeholders | Report before/after token values to the GM, caller, table, or controllers. |
|  | `!token-mod --ids <id...>` | `--ignore-selected`, `--current-page`, `--active-pages` | Add explicit token/character targets when authorized and optionally filter their pages. |
|  | `!token-mod --config players-can-ids|on|off` | GM only | Control whether players may supply explicit IDs; selected-token use remains available. |
...
| **TokenService** | `enabled` | bool | `true` | Enable general token controls and supported `!token-mod` compatibility. |
...
|  | `configSchemaVersion` | number | `1` | Protected TokenService configuration schema identifier. |
...
!token-mod --config players-can-ids|off
```
...
| **Token Service** | `GameAssist.TokenService` | Inspect component provenance/lifecycle and subscribe to token changes made through supported TokenService commands. |
...
When MarkerService is disabled, marker operations return `UNAVAILABLE` with the command needed to restore the service. ConditionService, TokenService, NPCManager, ConcentrationTracker, and DebugTools are disabled before the service closes so their teardown can complete safely. Observer registrations pause while the service is off and resume when it is enabled again.
...
`GameAssist.TokenService` is available for integrations that need TokenService lifecycle, provenance, or command-owned token-change notifications:

```js
const tokens = GameAssist.TokenService;

const subscription = tokens.observeTokenChange((token, previous, context) => {
    // Called after a supported TokenService command changes this token.
    // context.source === 'TokenService'; context.command contains the API command.
}, { owner: 'MyModule' });
...
| Method / Field | Result |
| --- | --- |
| `version` | TokenService component version (`1.0.0`). |
| `configSchemaVersion` | TokenService configuration schema (`1`). |
| `reference` | Pinned TokenMod reference version, repository, commit, path, and blob. |
| `isEnabled()` | Reports whether TokenService and MarkerService are both running. |
| `observeTokenChange(callback, options)` | Subscribes to successful TokenService command mutations and returns an unsubscribe handle. |
...
Use `GameAssist.MarkerService.observe(...)` when the integration needs every marker change, including direct GameAssist condition, death, concentration, or debug actions. Use TokenService observation only for complete token mutations performed by its supported command handler.
...
Disabling MarkerService also turns off ConditionService, TokenService, ConcentrationTracker, NPCManager, and DebugTools. Core admin commands remain available. NPCManager's configured marker may be cleared from current-page tokens, but its saved death-history and Arc records are retained.
...
### 12.8 TokenService Selected-Token Controls

```roll20chat
!token-service help
!token-mod --on showname
!token-mod --set bar1_value|-5
!token-mod --set statusmarkers|!red
```

Select disposable tokens first. The first command opens the guide; the remaining examples show a nameplate, subtract 5 from bar 1, and toggle the red marker through MarkerService.
...
ConditionService, TokenService, NPCManager, ConcentrationTracker, and DebugTools use `GameAssist.MarkerService`; they should report `deps confirmed` without standalone TokenMod or StatusInfo.
...
The details panel should report MarkerService as enabled. ConditionService and TokenService should appear enabled and running after standalone StatusInfo and TokenMod are removed. If a standalone script is detected, the details explain which overlapping command handler is suspended.
...
### 14.4 TokenService Does Not Respond or a Token Does Not Change
...
TokenService and MarkerService must both be running. If troubleshooting details report standalone TokenMod, remove that script and restart the sandbox; TokenService deliberately leaves `!token-mod` commands to the standalone handler while the collision exists.

Select a disposable token and try:

```roll20chat
!token-mod --on showname
!token-mod --set name|"TokenService Test"
```

Values containing spaces must be quoted. Players may use selected-token commands, but `--ids` remains restricted unless the GM enables it through `!token-mod --config players-can-ids|on`. Commands involving `imgsrc`, `sides`, `defaulttoken`, computed attributes, advanced color arithmetic, duplicate marker indexes, or conditional marker counts are outside TokenService 1.0.0's compatibility boundary and should produce a clear warning rather than a partial mutation.

### 14.5 ConditionService Does Not Respond, Shows the Wrong Wording, or Uses the Wrong Marker
...
MarkerService should report enabled. ConditionService, TokenService, NPCManager, and ConcentrationTracker should run with `deps confirmed`. Troubleshooting details should report that standalone TokenMod and StatusInfo were not detected.
...
Existing configured built-in names, custom display names, and exact stored tags remain supported. ConditionService copies valid legacy `state.STATUSINFO` settings and definitions into its own validated configuration while retaining the legacy branch for rollback. TokenService copies the valid legacy `playersCanUse_ids` choice and leaves `state.TokenMod` intact. Test every retained `!token-mod` macro against TokenService's documented compatibility boundary; unsupported advanced command families are reported rather than silently claimed.

V. **Run the Smoke Test**

Use [§4.1 Minimum Smoke Test](#41-minimum-smoke-test), including TokenService property and marker commands, real HP, concentration, marker preservation, MarkerService disable cascading, and service/module re-enablement.
...
| ConditionService 1.0.1 | **Implemented; sandbox acceptance pending** | Independently branded and attributed GameAssist condition service with `!condition` and case-insensitive `!cond-<condition>` references, selectable 2014/2024 SRD wording, campaign edits, marker artwork, verified marker-toggling announcements, validated legacy migration/import, and MarkerService synchronization. |
| TokenService 1.0.0 | **Implemented; sandbox acceptance pending** | Independently branded and attributed GameAssist token service with selected `!token-mod` compatibility, MarkerService-backed markers, practical legacy migration, observer replacement, explicit limits, and duplicate-install protection. |
...
* Added `[GAMEASSIST:MODULES:TOKENSERVICE]` and exposed `GameAssist.TokenService` with independently branded help, selected `!token-mod` compatibility, common token/bar/aura/vision/light/movement/report commands, explicit-ID authorization, non-destructive legacy configuration migration, and token-change observation.
* Pinned TokenService's TokenMod reference to release `0.8.88`, Roll20 repository commit `9d634d3149985dcf10333920b3f4c41f215f39fc`, and blob `fc6c9cb45ec2f2ee254a24f849e089507a0e610a`; preserved the applicable MIT notice and no-endorsement boundary.
* Routed every TokenService status-marker command through MarkerService and suspended `!token-mod` compatibility handling when standalone TokenMod is detected so a command cannot be applied twice.
...
* **TokenService**
  GameAssist's independently maintained general token-control module. It preserves a documented subset of `!token-mod` commands, delegates markers to MarkerService, and is not an official TokenMod release.
...
GameAssist v0.1.5.0 includes independently maintained TokenService and ConditionService modules under original GameAssist names.

For compatibility concepts or adapted upstream work, GameAssist retains:
...
**TokenService** acknowledges **TokenMod 0.8.88 by The Aaron** as its compatibility and design foundation. It preserves a documented subset of familiar `!token-mod` workflows while using original GameAssist lifecycle, validation, migration, collision handling, help, public API, and MarkerService integration. It is not an official TokenMod release and does not claim complete TokenMod compatibility.

**ConditionService** acknowledges **StatusInfo by Robin Kuiper** as its compatibility and design foundation. It preserves selected `!condition` workflows and legacy configuration concepts while using original GameAssist lifecycle, validation, menus, migration, wording profiles, and MarkerService integration. It is not an official StatusInfo release.
