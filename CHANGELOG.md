...
- Builds the Issue #96 code program in order, currently including the generic immutable PresetRegistry and editable campaign Session Preset clones alongside the previously implemented SceneResolver, Worldbuilding, Travel, and Phenomena foundations.
- Defers live Roll20 validation until the full Issue #96 code program—including RulesAdvisor, WorldPacks, and temporal contexts—is built. Focused automated checks remain development evidence only.
