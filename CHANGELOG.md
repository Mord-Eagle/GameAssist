...
- Adds `tests/almanac-starter-worlds.test.js` and focused Gate 0 harness coverage for first-run routes, decoded generated commands, visible first-level query choices, generated refusal/no-change recovery controls, active world switching, Climate prompt contracts, first Location activation, named page assignment, compatible WorldPack/Wayfarer handout pickers, and no-raw-handout-ID paths. Adds `tests/almanac-rest.test.js` for selected linked-2014-PC preview, accepted Long Rest, stale-plan, and controller-boundary behavior. These tests do not replace the required real Roll20 render/prompt recovery pass.
