...
Issue #26 requires a Roll20 Mod sandbox pass with standalone StatusInfo absent. The pass must cover ConditionAssist help/menu output, case-insensitive official/custom `!cond-<condition>` references, the `!c-a` and `!cond-!` aliases, 2014/2024/custom wording changes, Concentrating display repair, built-in and registered custom artwork, selected-character announcement marker toggling, character-first is/is-no-longer reporting, public and controller-targeted destinations, duplicate-description suppression, private-reference buttons, preservation of custom definitions and marker choices, permissions, descriptions, add/remove/toggle, duplicate-marker warnings, built-in and numbered custom markers, direct marker-change observation, schema-v2 import/export, legacy-state migration when available, MarkerService cascade disable/re-enable, and duplicate-install warnings.

Issue #27 requires a Roll20 Mod sandbox pass with standalone TokenMod absent. The pass must cover TokenAssist help/About, `!token-assist` and `!ta`/`!ta-*`, legacy-alias deprecation, selected-token booleans and values, a visible aura, quoted text, relative bar and position changes, movement without a stale trail origin, order, reports, linked bars, built-in/custom/numbered markers, marker replacement safety, player selected-token use, player `--ids` refusal/opt-in, current-page filtering, unsupported-feature diagnostics, observer delivery where testable, legacy `playersCanUse_ids` and unreleased-name migration, MarkerService cascade disable/re-enable, sandbox reload, and accidental standalone TokenMod alias suspension in a disposable collision test.
