...
- Builds the Issue #96 code program in order, currently including the generic immutable PresetRegistry, editable campaign Session Preset clones, optional profile-specific advisory-only RulesAdvisor, and bounded atomic WorldPacks alongside the previously implemented SceneResolver, Worldbuilding, Travel, and Phenomena foundations.
- Defers live Roll20 validation until the full Issue #96 code program—including WorldPacks and temporal contexts—is built. Focused automated checks remain development evidence only.
