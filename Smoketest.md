...
4. Run the complete [Focused v2.0.0 HealthService Acceptance](#focused-v200-healthservice-acceptance) section. It may not be skipped for release approval.
...
| Focused HealthService acceptance passes | ☐ Pass ☐ Fail |
...
Pass when all three commands whisper only the GM, the audit says it made no changes, and an empty recent list is described as no evidence yet rather than an error.
