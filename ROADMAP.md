...
| AlmanacAssist custom-calendar and world-context completion | In progress | [#90](https://github.com/Mord-Eagle/GameAssist/issues/90), [#95](https://github.com/Mord-Eagle/GameAssist/issues/95) | AlmanacAssist 1.10.0 on the `AlmanacAssist-v2.0.0-Build` line retains the Issue #89 action-first checkpoint and Gate 0 repairs, then adds the read-only SceneResolver, bounded generic Worldbuilding place composition, active Location/favorites/recents, Location-bound Prepared Destinations, reviewed route/pace Travel, and explicit reviewed Phenomena overlays. v2.0.0 still requires full live verification, presets, portable WorldPacks, temporal contexts, and the complete Issue #90 acceptance track. |
...
- [x] On `AlmanacAssist-v2.0.0-Build`, accept Issue #92 (remove the redundant full-range chronology pre-scan and unused `maximumWorldMinute()` helper) and Issue #93 (make `getSubmoduleStatus()` consistently report configured subsystem state). AlmanacAssist advanced to `1.6.2` on that build line; the subsequent 1.7.0 checkpoint adds a read-only SceneResolver foundation and focused VM coverage; 1.8.0 adds bounded generic Worldbuilding place composition; and 1.9.0 adds Prepared Destinations plus reviewed Travel with focused VM coverage; 1.10.0 adds explicit bounded Phenomena overlays with focused VM coverage.
