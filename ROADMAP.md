...
AlmanacAssist 2.0.4 adds **Current Settings**: a regional baseline, an editable twelve-profile local influence matrix, explicit GM adjustments, and independent named-location snapshots. The GM can prepare weather inputs without building a hierarchy, save a place, and recall it later without changing time or restoring an old weather roll. Existing linked-world controls and saved data remain available; broader cleanup follows live acceptance of the simpler workflow.

The next live check is **Current Settings And Location Recall** in `Smoketest.md`, followed by the retained **Location Setup And Preservation** and **World Profiles And Seasonal Weather** checks. Focused automation covers the new settings, weather calculations, save/recall isolation, WorldPack transfer, permissions, and restart preservation alongside the existing Almanac regression checks. Local results do not close the Roll20 live acceptance gate.
