...
| AttackAssist guided attacks | Sandbox verification | [#87](https://github.com/Mord-Eagle/GameAssist/issues/87) | AttackAssist 1.0.4 adds authorized official-2014 repeating-attack selection, stable row identity, a compact native visible-target choice, retained private placement requests, executable explicit roll modes, documented Classic-sheet attack-template defaults, a preflight refusal for genuinely unknown missing fields, one-use submission, and no damage or combat-state writes. |
...
4. **Issue #87 — AttackAssist:** AttackAssist 1.0.4 is implemented at sandbox verification with compact direct attack-row target choices, explicit modes, documented Classic-sheet checkbox defaults, and preflight refusal for genuinely unknown missing fields; complete the live source, duplicate-row, target, roll-mode, natural-1, privacy, stale-button, Companion coexistence, and lifecycle track before closing.
