...
| AlmanacAssist v2.0.0 custom-calendar and world-context completion | In progress | [#90](https://github.com/Mord-Eagle/GameAssist/issues/90), [#95](https://github.com/Mord-Eagle/GameAssist/issues/95), [#96](https://github.com/Mord-Eagle/GameAssist/issues/96) | AlmanacAssist **v2.0.0** on the `AlmanacAssist-v2.0.0-Build` line retains the Issue #89 action-first checkpoint and Gate 0 repairs, then builds the read-only SceneResolver, bounded generic Worldbuilding place composition, active Location/favorites/recents, Location-bound Prepared Destinations, reviewed route/pace Travel, explicit reviewed Phenomena overlays, generic PresetRegistry campaign clones, optional profile-specific advisory-only RulesAdvisor reminders, and bounded atomic WorldPacks with inert handout template/export/import, provenance, stale previews, New/Update/Copy policy, and focused VM coverage. Temporal contexts remain code work. Live Roll20 validation and the Issue #90 acceptance track are deferred until the full Issue #96 code build is complete. |
...
**AlmanacAssist-v2.0.0-Build branch:** Issue #95 is the implementation master for the 2.0 engine and Issue #94 is the architectural destination. The Gate 0 repairs from Issues #92 and #93 are implemented on the duplicate `AlmanacAssist-v2.0.0-Build` branch, with the issue assessment, comparison, guardrails, and Gate-by-Gate plan under `docs/AlmanacAssist-v2.0.0-Build/`.
...
- [x] On `AlmanacAssist-v2.0.0-Build`, accept Issue #92 (remove the redundant full-range chronology pre-scan and unused `maximumWorldMinute()` helper) and Issue #93 (make `getSubmoduleStatus()` consistently report configured subsystem state). Historical internal checkpoints 1.6.2 through 1.10.0 record the Gate 0 repair, SceneResolver, Worldbuilding, Travel, and Phenomena implementation sequence; the active AlmanacAssist implementation version is **v2.0.0**. Focused VM coverage remains development evidence, while live Roll20 validation is deferred until the entire Issue #96 code program is built.
- [x] Pass 107 focused AlmanacAssist checks, 36 focused Wayfarer setup checks, 109 EffectAssist regression checks, 70 focused HealthService transitions, the 14-check interaction/Almanac repair harness, JavaScript syntax parsing, and the 32-section MECHSUITS structural validator.
