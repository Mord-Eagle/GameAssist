...
| Wayfarer Calendar Manager | Sandbox verification | [#89](https://github.com/Mord-Eagle/GameAssist/issues/89) | AlmanacAssist 1.2.0 adds direct editors for every calendar component, retained guided review, editable seasonal ranges, visible moon phases, focused aliases, atomic activation, elapsed-time preservation, command-only draft recovery, and one rollback point. |
...
| Guidance verified consumption | Sandbox verification | [#85](https://github.com/Mord-Eagle/GameAssist/issues/85) | EffectAssist 2.4.0 labels its owned Guidance row and may consume one exact, unambiguous supported 2014 skill check. Unsupported or ambiguous evidence remains manual; live normal/advantage/disadvantage acceptance is still required. |
...
1. **Issue #89 — Wayfarer Calendar Manager:** AlmanacAssist 1.2.0 is implemented at sandbox verification; complete the live direct-editor, seasonal-range, moon-phase, alias, activation, rollback, and reset-recovery track before closing.
...
5. **Issue #85 — Guidance consumption:** EffectAssist 2.4.0 implements the guarded candidate using an exact owned roll label, current projection ownership, controller checks, unique character/instance resolution, normal lifecycle cleanup, and duplicate-event suppression. Complete the focused live normal, advantage, disadvantage, unrelated-d4, ambiguity, and manual-fallback checks before closing it.
