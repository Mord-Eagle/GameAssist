...
| AlmanacAssist custom-calendar and world-context completion | In progress | [#90](https://github.com/Mord-Eagle/GameAssist/issues/90), [#95](https://github.com/Mord-Eagle/GameAssist/issues/95) | AlmanacAssist 1.6.2 on the `AlmanacAssist-v2.0.0-Build` line retains the Issue #89 action-first control checkpoint and full-name command consistency and accepts the Gate 0 repairs from Issues #92 and #93, but v2.0.0 still requires a usable complete Wayfarer editor, reliable query-driven customization, coherent climate/weather/environment ownership, moon and rest controls, default recovery, the Issue #94 SceneResolver engine, and the full live Roll20 acceptance track. |
...
- [x] On `AlmanacAssist-v2.0.0-Build`, accept Issue #92 (remove the redundant full-range chronology pre-scan and unused `maximumWorldMinute()` helper) and Issue #93 (make `getSubmoduleStatus()` consistently report configured subsystem state). AlmanacAssist advanced to `1.6.2` on that build line.
