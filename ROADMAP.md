...
## Current Almanac Repair

AlmanacAssist 2.0.3 repairs numeric palette-choice identifiers and keeps a chosen destination attached to its profile buttons. Locations now have direct naming, one-prompt creation, pre-travel profile setup, compact saved-place lists, and separate detailed/travel controls. Existing locations are preserved, and an empty prepared list is no longer repopulated automatically. The existing WorldPack handout remains the advanced location/world editor.

The reported live palette-button failure requires a fresh click-through check using **Location Setup And Preservation** and **World Profiles And Seasonal Weather** in `Smoketest.md`. The 195-check Almanac harness, 48 focused palette/location checks, 36 Wayfarer checks, and 20 interaction-repair checks pass locally. These do not close the live acceptance gate.
...
| AlmanacAssist master program | Expanded profile checks passed; live acceptance pending | [#95](https://github.com/Mord-Eagle/GameAssist/issues/95), [PR #81](https://github.com/Mord-Eagle/GameAssist/pull/81) | AlmanacAssist 2.0.3 adds reusable natural-world profile palettes and season-aware weather to the reviewed travel layer on the consolidated engine and all six internal systems; publication waits for the complete documented Roll20 acceptance track. |
...
| AlmanacAssist coherent world engine | Automated gates passed; live acceptance pending | [#95](https://github.com/Mord-Eagle/GameAssist/issues/95) | AlmanacAssist 2.0.3 adds compact weather/time/travel/event/announcement/setup palettes and stale-safe mileage travel with advisory road conditions and GM-private encounter checks while retaining Session and Worldbuilding modes, signed Year 0 chronology, a provenance-bearing SceneResolver, bounded definitions, prepared destinations, phenomena, local temporal contexts, presets, RulesAdvisor, advanced Wayfarer editing, and atomic WorldPacks. The focused local gate passes 195 checks; live Roll20 evidence is still outstanding. |
...
- [x] Pass 195 focused AlmanacAssist checks, 36 focused Wayfarer setup checks, 134 EffectAssist regression checks, 70 focused HealthService transitions, the 20-check interaction/Almanac repair harness, JavaScript syntax parsing, and the 33-section MECHSUITS structural validator. All 13 local harnesses pass; these remain non-live evidence.
...
v2.0.0 does not publish until every internal system and Issue #95 world-engine workflow passes the focused Roll20 smoke track, GM-private PC threshold behavior passes Issue #86, guided HealAssist review and verified application passes Issue #84, guided AttackAssist source/target/roll/privacy behavior passes Issue #87, independent disable/re-enable and restart preservation are confirmed, RestAlmanac's supported 2014-sheet writes are proven with disposable characters, and PR #81 review is complete. Passing the 195-check local Almanac gate, TimeAlmanac alone, or any smaller subset is not a release candidate.
