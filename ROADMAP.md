...
Use this document for durable release boundaries, sequencing, and completion gates. Use the linked GitHub issues for implementation details and acceptance evidence. Issues #60, #64, and #65 are complete. PR #81 remains the single v2.0.0 development line: EffectAssist, HealAssist, AttackAssist, and AlmanacAssist share that branch; Issue #88 carries the EffectAssist player-casting and retained GM-request workflow; and Issue #95 governs AlmanacAssist as the consolidated world-engine specification. AlmanacAssist 2.0.5 is designated **Beta Testing** for inclusion in v2.0.0, with campaign-scale GM palettes and reviewed mileage travel. Its focused 195-check non-live gate passes for compact session controls, SceneResolver, Year 0 compatibility, world definitions, destination and mileage travel, private encounter checks, temporal contexts, phenomena, presets, Wayfarer handout transactions, WorldPacks, and lifecycle preservation. The owner accepted live Almanac use for beta on 2026-08-27; the subsequent grouped local sweep passed all 18 runs. Expanded campaign and upgrade verification continues without presenting untested cases as passed. SheetCapabilities 1.0.0 implements the first Issue #91 contract for operation-specific 2014, 2024/Beacon, and unknown evidence; later verified adapters remain phased work. Issues #83, #79, #80, #77, #84, and #87 retain their accepted v2.0.0 integration scope. None of this work creates a parallel release branch.
...
**AlmanacAssist 2.0.5: Beta Testing.** **Current Settings** provides direct Climate and Biome choices, twelve ecoregion starters, and optional Geography, Terrain, Environment, Hydrology, and Vegetation layers. Named buttons are paginated; Seasons and Weather Breakdown expose the actual influence on generation. Saved locations and custom local profiles retain the layered setup without changing time or restoring an old weather roll. Existing linked-world controls and saved data remain available under advanced tools. All six internal systems are included; the beta remains optional and disabled by default while broader campaign testing continues.
...
| Beta Testing | Available as an explicitly labeled optional feature while broader campaign testing continues; not a claim of full acceptance or an exemption from privacy and data-safety checks. |
...
13. **v2.0.0 includes all six AlmanacAssist systems under Beta Testing.** Time, Climate, Astronomy, Weather, Environment, and Rest remain independently controlled within one optional module. None ships as a separate partial release, and none becomes a hidden prerequisite for unrelated GameAssist modules.
...
| AlmanacAssist master program | **Beta Testing** | [#95](https://github.com/Mord-Eagle/GameAssist/issues/95), [PR #81](https://github.com/Mord-Eagle/GameAssist/pull/81) | AlmanacAssist 2.0.5 includes all six systems, layered world profiles, season-aware weather, and reviewed travel as an optional beta in v2.0.0. Expanded live and upgrade results remain part of beta follow-up, not falsely completed gates. |
| TimeAlmanac | Beta Testing | [#66](https://github.com/Mord-Eagle/GameAssist/issues/66) | One elapsed fictional-minute authority, four profiles, editable Wayfarer structure and holidays, guarded changes, player read-only output, history, and events. |
| ClimateAlmanac | Beta Testing | [#67](https://github.com/Mord-Eagle/GameAssist/issues/67) | Built-in/editable/custom profiles, bounded regions, parent inheritance, overrides, active region, and manual season fallback. |
| AstronomyAlmanac | Beta Testing | [#68](https://github.com/Mord-Eagle/GameAssist/issues/68) | Configurable moons/phases, daylight and deterministic season boundaries, read-only forecasts, and separate weighted rare-event suggestions. |
| WeatherAlmanac | Beta Testing | [#69](https://github.com/Mord-Eagle/GameAssist/issues/69) | Structured continuity-aware current weather, optional context, fallback operation, forecasts, locks, manual weather, history, and events. |
| EnviroAlmanac | Beta Testing | [#70](https://github.com/Mord-Eagle/GameAssist/issues/70) | Structured descriptive environment context, weather derivation, manual presets, persistent override, and no automatic gameplay penalties. |
| RestAlmanac | Beta Testing | [#71](https://github.com/Mord-Eagle/GameAssist/issues/71) | Previewed and revalidated 2014-PC Short/Long/Extended/custom rests with verified fields, optional Time advancement, rollback, history, and events. |
| AlmanacAssist coherent world engine | **Beta Testing; local gates passed** | [#95](https://github.com/Mord-Eagle/GameAssist/issues/95) | AlmanacAssist 2.0.5 adds compact weather/time/travel/event/announcement/setup palettes and stale-safe mileage travel with advisory road conditions and GM-private encounter checks while retaining Session and Worldbuilding modes, signed Year 0 chronology, a provenance-bearing SceneResolver, bounded definitions, prepared destinations, phenomena, local temporal contexts, presets, RulesAdvisor, advanced Wayfarer editing, and atomic WorldPacks. Owner-reported live use was accepted for beta; expanded verification continues. |
...
- [x] Record owner-reported live Almanac use accepted for beta on 2026-08-27, without treating it as a completed case-by-case acceptance inventory.
...
AlmanacAssist may be included in GameAssist v2.0.0 as an explicitly labeled, optional **Beta Testing** module. Its complete expanded smoke track remains the graduation target, not a requirement to claim every beta workflow fully accepted before inclusion. The beta baseline covers startup, basic navigation, current settings, generation, persistence, and the safety of enabled integrations; RestAlmanac sheet writes must be checked with disposable characters when enabled. A confirmed sandbox crash, privacy leak, data loss, or unsafe write remains a release blocker. Other modules retain their own release checks, and PR review plus final package verification remain separate publishing steps.
...
1. **Issue #95 - AlmanacAssist 2.0.5 Beta Testing:** include the optional module in v2.0.0 with its beta baseline and clear labeling. Continue expanded live coverage for Session Mode, Worldbuilding Mode, coherent scenes, Year 0, travel, local time, phenomena, Wayfarer handout editing, WorldPacks, all six subsystem boundaries, rest writes, restart preservation, and UI readability before graduating it from beta.
...
Completed checkpoints must satisfy the applicable checks below. An explicitly labeled beta may retain documented expanded testing, but privacy, data preservation, integration safety, and accurate package information remain release requirements. AlmanacAssist's beta designation does not mark other outstanding checks as passed:
