...
Use this document for durable release boundaries, sequencing, and completion gates. Use the linked GitHub issues for implementation details and acceptance evidence. Issues #60, #64, and #65 are complete. PR #81 remains the single v2.0.0 development line: EffectAssist, HealAssist, AttackAssist, and all six AlmanacAssist systems are implemented checkpoints; Issue #88 carries the player-casting and retained GM-request workflow now advanced through EffectAssist 2.5.0 with separate compact caster and recipient-count steps, Bless totals from one through eleven, and consistent `(GA)` sheet labels; Issue #89 carries AlmanacAssist 1.6.0 with independent per-field announcement presentation, a real generic climate seed, coherent current-scene versus stored-weather ownership, repaired calendar-query controls, and focused Wayfarer calendar management; Issue #83 adds the shared HealthService foundation; Issue #79 uses that foundation for private concentration-check offers now tolerant of equivalent linked evidence; Issue #80 adds conservative effect-duration candidates; Issue #77 adds bounded official 2014 Bless cast proposals; Issue #84 adds reviewed 2014 healing through verified HealthService writes; and Issue #87 advances AttackAssist to 1.0.2 with compact direct targeting and executable roll modes that do not depend on a saved `d20` attribute. None of this work creates a parallel release branch or authorizes publication.
...
| EffectAssist player-casting workflow | Sandbox verification | [#88](https://github.com/Mord-Eagle/GameAssist/issues/88) | EffectAssist 2.5.0 uses short-lived player-bound choices, a compact caster step followed by recipient count, Bless totals of 1-3 plus Higher Level Casting totals of 4-11, consistent `(GA)` owned rows, retained GM requests, direct GM controls, and visible stale-button recovery without exposing source identifiers in player buttons. |
...
| AlmanacAssist daily controls and Wayfarer Calendar Manager | Sandbox verification | [#89](https://github.com/Mord-Eagle/GameAssist/issues/89) | AlmanacAssist 1.6.0 adds an action-first GM dashboard, Wayfarer-native ordinal Hour and daily-period presentation, calendar-aware advance/set controls, independent Off/Descriptive/Detailed/Technical choices for every announcement field, consistent scene/weather/climate ownership, daylight/cloud-aware moon visibility, direct saved-Wayfarer and Astronomy editors, configurable rest rules, optional teaching/detail screens, retained guided review, atomic activation, elapsed-time preservation, command-only draft recovery, and one rollback point. |
...
| AttackAssist guided attacks | Sandbox verification | [#87](https://github.com/Mord-Eagle/GameAssist/issues/87) | AttackAssist 1.0.2 adds authorized official-2014 repeating-attack selection, stable row identity, a compact native visible-target choice, retained private placement requests, executable explicit roll modes that materialize their d20 expressions, one-use submission, and no damage or combat-state writes. |
| Guidance verified consumption | Sandbox verification | [#85](https://github.com/Mord-Eagle/GameAssist/issues/85) | EffectAssist 2.5.0 retains the compact owned Guidance label and may consume one exact, unambiguous supported 2014 skill check. Unsupported or ambiguous evidence remains manual; live normal/advantage/disadvantage acceptance is still required. |
...
- [x] Add documentation, configuration and observational public API references, One-Click metadata, focused smoke tests, a 55-check local regression harness, and focused direct-target route coverage.
...
1. **Issue #89 — AlmanacAssist daily controls and Wayfarer Calendar Manager:** AlmanacAssist 1.6.0 is implemented at sandbox verification; complete the live action-first dashboard, Wayfarer's ordinal 20-hour presentation, active-calendar time bounds, independent field presentation, audience/preset/heading controls, direct saved-Wayfarer and Astronomy editing, configurable rest rules, coherent scene/weather/climate output, seasonal-range, moon-visibility, alias, activation, rollback, and reset-recovery track before closing.
...
4. **Issue #87 — AttackAssist:** AttackAssist 1.0.2 is implemented at sandbox verification with compact direct attack-row target choices and explicit modes that do not depend on a saved `d20` attribute; complete the live source, duplicate-row, target, roll-mode, natural-1, privacy, stale-button, and lifecycle track before closing.
5. **Issue #85 — Guidance consumption:** EffectAssist 2.5.0 retains the guarded candidate using an exact compact owned roll label, current projection ownership, controller checks, unique character/instance resolution, normal lifecycle cleanup, and duplicate-event suppression; player casting now separates caster and recipient-count choices. Complete the focused live normal, advantage, disadvantage, unrelated-d4, ambiguity, and manual-fallback checks before closing it.
