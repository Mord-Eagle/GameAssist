...
| AlmanacAssist v2.0.0 custom-calendar and world-context completion | Sandbox verification | [#90](https://github.com/Mord-Eagle/GameAssist/issues/90), [#95](https://github.com/Mord-Eagle/GameAssist/issues/95), [#96](https://github.com/Mord-Eagle/GameAssist/issues/96) | AlmanacAssist **v2.0.0** on the `AlmanacAssist-v2.0.0-Build` line has the complete Issue #96 code build: Gate 0 repairs; read-only SceneResolver; bounded generic Worldbuilding with Basic/Detailed/Technical record editors; active Location/favorites/recents; Prepared Destinations; reviewed route/pace Travel; reviewed Phenomena; PresetRegistry; advisory-only RulesAdvisor; bounded atomic WorldPacks; explicit reviewed Temporal Contexts; future Worldbuilding runtime preservation; and the optional versioned Wayfarer handout export/edit/import path. The consolidated automated checkpoint passed on 2026-08-26; the final live Roll20/Issue #90 acceptance track remains required, and no release claim is made. |
...
**AlmanacAssist-v2.0.0-Build branch:** Issue #95 remains the implementation master for the 2.0 engine and Issue #94 is the architectural destination. The duplicate build tracker is #96; the Gate 0 repairs from Issues #92 and #93 and every later code gate are implemented on the `AlmanacAssist-v2.0.0-Build` line. The issue assessment, migration review, guardrails, and Gate-by-Gate plan live under `docs/AlmanacAssist-v2.0.0-Build/`. Final live Roll20 validation remains separate and is not represented here as complete.
...
- [x] Implement the remaining #96 code gates: WorldPackService; explicit Prime/regional/planar Temporal Contexts with reviewed canonical-time reconciliation and bounded committed events; and consistent Basic/Detailed/Technical Worldbuilding record-editor layers with technical provenance/stable identity.
...
- [x] Add focused VM coverage for temporal contexts, optional Wayfarer handout export/import, Worldbuilding editor layers, WorldPacks, migration/future-schema preservation, and prior Almanac boundaries. These are development evidence, not a release gate.
